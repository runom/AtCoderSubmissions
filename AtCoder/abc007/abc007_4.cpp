...
int64_t f(string a) {
    ...
    for (int i : rep(0, a[0] - '0' + 1)) {
        if (i == 4 || i == 9) {
            continue;
        }
        dp[i == a[0] - '0'][a.size() - 1] += 1;
    }

    int al = a.size();

    for (int i : rep(1, a.size()) | reversed) {
        for (int k : rep(0, 10)) {
            if (k == 4 || k == 9) continue;
            dp[0][i - 1] += dp[0][i];

            if (k <= a[al - 1 - (i - 1)] - '0') {
                dp[k == a[al - 1 - (i - 1)] - '0'][i - 1] += dp[1][i];
            }
        }
    }

    return stoll(a) - (dp[0][0] + dp[1][0]) + 1;
}
...
int main() {
    ...
    out(f(to_string(b)) - f(to_string(a - 1)));
}