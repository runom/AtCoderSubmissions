...
int main() {
    ...
    auto v = vin<int64_t>(n);

    int64_t comb[51][51] = {0};
    comb[0][0] = 1;
    for (int i : rep(1, n + 1)) {
        for (int j : rep(0, i + 1)) {
            if (j == 0 || j == i) {
    ...
    long double ave = 1.0L * accumulate(v | sliced(0, a), 0LL) / a;

    int64_t cnt = 0;

    for (int i : rep(a, b + 1)) {
        if (ave != 1.0L * accumulate(v | sliced(0, i), 0LL) / i) {
            break;
        }
