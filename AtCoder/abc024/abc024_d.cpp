...
int64_t pow_mod(int64_t base, int64_t n, int64_t mod) {
    int64_t res = 1;
...
int64_t inverse(int64_t x, int64_t mod) { return pow_mod(x, mod - 2, mod); }
...
int main() {
    ...
    int64_t r =
        (B * C % mod + mod - A * C % mod) % mod *
        inverse((A * B % mod + mod - (B * C) % mod + A * C % mod) % mod, mod) %
        mod;

    int64_t c =
        (B * C % mod + mod - A * B % mod) % mod *
        inverse((A * C % mod + mod - B * C % mod + A * B % mod) % mod, mod) %
        mod;
