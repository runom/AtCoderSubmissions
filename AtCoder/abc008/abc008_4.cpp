...
int dfs(Point tl, Point br) {
    ...
    int res = 0;
    for (auto &p : xy) {
        if (tl.first <= p.first && p.first < br.first &&
            tl.second <= p.second && p.second < br.second) {
            res = max(res,
                      dfs(tl, {p.first, p.second}) +
                          dfs({p.first + 1, tl.second}, {br.first, p.second}) +
                          dfs({tl.first, p.second + 1}, {p.first, br.second}) +
                          dfs({p.first + 1, p.second + 1}, br) +
                          (br.first - tl.first) + (br.second - tl.second) - 1);
        }
    }

    return memo[{tl, br}] = res;
}
