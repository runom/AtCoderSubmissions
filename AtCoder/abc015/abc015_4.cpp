...
const int max_n = 50, max_k = 50, max_w = 10000;

int memo[max_n + 1][max_k + 1][max_w + 1];
...
int dfs(int n, int k, int w) {
    ...
    if (w - a[n - 1] >= 0) {
        res = max(res, dfs(n - 1, k - 1, w - a[n - 1]) + b[n - 1]);
    }

    return memo[n][k][w] = res;
}
...
int main() {
    ...
    fill((int *)memo, (int *)(memo + max_n + 1), -1);
