...
int main() {
    ...
    int64_t comb[1000][1000];
    comb[0][0] = 1;
    for (int i : rep(1, 1000)) {
        for (int j : rep(0, i + 1)) {
            if (j == 0 || j == i) {
                comb[i][j] = 1;
            } else {
    ...
    for (int i : rep(0, 1 << 4)) {
        ...
        if (xx <= 0 || yy <= 0 || xx * yy - d < 0) {
            continue;
        }

        if (bitset<4>(i).count() % 2 == 0) {
            sum = (sum + comb[xx * yy][d] * comb[xx * yy - d][l] % mod) % mod;
        } else {
            sum = (sum + mod - comb[xx * yy][d] * comb[xx * yy - d][l] % mod) %
                  mod;
        }
    }

    out((r - x + 1) * (c - y + 1) * sum % mod);
}