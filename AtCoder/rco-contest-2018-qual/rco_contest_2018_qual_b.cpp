...
int main() {
    ...
    vector<vector<int>> sector(h, vector<int>(w, -1));
    ...
    for (int i : rep(0, k)) {
        ...
        int t = 0;
        for (int j : rep(0, d - 2)) {
            int cost = abs(r[j] - r[j + 1]) + abs(c[j] - c[j + 1]) +
                       abs(r[j + 1] - r[j + 2]) + abs(c[j + 1] - c[j + 2]);
        ...
        out(sr, sc, r[t + 1], c[t + 1]);

        if (sector[sr][sc] != -1) {
            swap(r[sector[sr][sc]], r[t + 1]);
            swap(c[sector[sr][sc]], c[t + 1]);
        ...
        swap(sector[sr][sc], sector[r[t + 1]][c[t + 1]]);
    }
}