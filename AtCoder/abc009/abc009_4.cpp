...
vector<vector<uint32_t>> mat_mul(const vector<vector<uint32_t>> &a,
                                 const vector<vector<uint32_t>> &b) {
    auto res = make_vector<uint32_t, 2>(a.size(), b[0].size());

    for (int i : rep(0, a.size())) {
        for (int j : rep(0, b[0].size())) {
            for (int k : rep(0, a[0].size())) {
                res[i][j] += a[i][k] * b[k][j];
            }
        }
    }
...
uint32_t matrix_pow(vector<vector<uint32_t>> A, const vector<uint32_t> &x,
                    int m) {
    int n = A.size();
    auto Am = make_vector<uint32_t, 2>(n, n);
    ...
    while (m > 0) {
        if (m & 1) {
            auto tmp = make_vector<uint32_t, 2>(n, n);

            for (int i : rep(0, n)) {
                for (int j : rep(0, n)) {
                    for (int k : rep(0, n)) {
                        tmp[i][j] ^= Am[i][k] & A[k][j];
                    }
                }
            }
    ...
    for (int i : rep(0, Am.size())) {
        res ^= Am[Am.size() - 1][i] & x[i];
    }
...
int main() {
    ...
    auto a = vin<uint32_t>(k);
    auto c = vin<uint32_t>(k);
    ...
    if (m < k) {
        ...
        uint32_t ans = 0;
        auto A = make_vector<uint32_t, 2>(k, k);
        vector<uint32_t> x(k);

        for (uint32_t i : rep(1, k)) {
            A[i - 1][i] = ~0u;
        }

        for (uint32_t i : rep(0, k)) {
            A[k - 1][i] = c[i];
            x[i] = a[i];
        }
        out(matrix_pow(A, x, m - k));
    }
}