...
pair<Res, bool> memo[53][53][53];

Res f(int l, int r, int k) {
    if (l == r) {
        if (k > 0) {
            return {0, 9};
        } else if (isdigit(s[l])) {
    ...
    if (memo[l][r][k].second) {
        return memo[l][r][k].first;
    }

    if (!isdigit(s[r])) {
        for (int i : rep(l, r - 1, 2)) {
            for (int j : rep(0, k + 1)) {
                auto r1 = f(l, i, j);
                auto r2 = f(i + 1, r - 1, k - j);

                if (r1.mini == inf || r2.mini == inf) {
                    continue;
                }

                if (s[r] == '+') {
                    ret.mini = min(ret.mini, r1.mini + r2.mini);
                    ret.maxi = max(ret.maxi, r1.maxi + r2.maxi);
                } else {
                    ret.mini = min(ret.mini, r1.mini - r2.maxi);
                    ret.maxi = max(ret.maxi, r1.maxi - r2.mini);
                }
            }
        }
    }
