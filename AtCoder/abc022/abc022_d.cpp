...
struct Vec2 {
    ...
    bool operator==(Vec2 vec) const { return x == vec.x && y == vec.y; }

    Vec2 operator+(Vec2 vec) const { return {add(x, vec.x), add(y, vec.y)}; }

    Vec2 operator-(Vec2 vec) const { return {add(x, -vec.x), add(y, -vec.y)}; }

    Vec2 operator*(type d) const { return {x * d, y * d}; }

    type dot(Vec2 vec) const { return add(x * vec.x, y * vec.y); }

    type det(Vec2 vec) const { return add(x * vec.y, -y * vec.x); }
};

using Point = Vec2<double>;

inline std::vector<Point> ConvexHull(std::vector<Point> ps, int n) {
...
int main() {
    ...
    const auto ach = ConvexHull(a, n);
    const auto bch = ConvexHull(b, n);

    double amin = 1e10;

    for (int i : rep(0, ach.size())) {
        for (int j : rep(i + 1, ach.size())) {
            amin = min(amin, sqrt(pow(ach[i].x - ach[j].x, 2) +
                                  pow(ach[i].y - ach[j].y, 2)));
        }
    }

    double bmin = 1e10;
    for (int i : rep(0, bch.size())) {
        for (int j : rep(i + 1, bch.size())) {
            bmin = min(bmin, sqrt(pow(bch[i].x - bch[j].x, 2) +
                                  pow(bch[i].y - bch[j].y, 2)));
        }
    }
