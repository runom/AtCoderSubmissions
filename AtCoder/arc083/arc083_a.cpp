...
int main() {
    ...
    for(int ic = 0; ic * c <= f; ++ic) {
        for(int id = 0; ic * c + id * d <= f; ++id) {
            s.push_back(ic * c + id * d);
        }
    }
    ...
    for(int ia : rep(0, 31)) {
        for(int ib : rep(0, 31)) {
            int ab = ia * a * 100 + ib * b * 100;

            if(ab > f) {
                continue;
            }

            int cd = *(upper_bound(s, min(f - ab, ab * e / 100)) - 1);

            if(ans.second * (ab + cd)  < cd * (ans.first + ans.second)) {
                ans.first = ab;
                ans.second = cd;
            }
        }
