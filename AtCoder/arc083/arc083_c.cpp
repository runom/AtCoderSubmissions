...
int subset_sum(const vector<int> &A, int X) {
    vector<int> dp(X + 1);

    dp[0] = 1;
    for (auto a : A) {
        for (int i : rep(0, X + 1) | reversed) {
            if (i - a >= 0 && dp[i - a]) {
                dp[i] = 1;
            }
        }
...
pair<int, int> dfs(int node) {
    ...
    for (int to : g[node]) {
        ...
        c.push_back(ret.second - ret.first);

        sum1 += ret.first;
        sum2 += ret.first + ret.second;
    }
    ...
    int res2 = sum2 - subset_sum(c, x[node] - sum1) - sum1;
...
int main() {
    ...
    out(ret.first != inf ? "POSSIBLE" : "IMPOSSIBLE");
}
