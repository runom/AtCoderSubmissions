...
int main() {
    ...
    for (int i : rep(0, N)) {
        ...
        if ((x1 == 0 || x1 == R || y1 == 0 || y1 == C) &&
            (x2 == 0 || x2 == R || y2 == 0 || y2 == C)) {
            a.emplace_back(make_pair(x1, y1), i);
            a.emplace_back(make_pair(x2, y2), i);
        }
    }
    ...
    sort(a, [&](auto x, auto y) {
        ...
        if (x.first.first == 0) {
            xc = x.first.second;
        } else if (x.first.second == C) {
            xc = C + x.first.first;
        } else if (x.first.first == R) {
            xc = R + C + (C - x.first.second);
        } else if (x.first.second == 0) {
            xc = R + 2 * C + (R - x.first.first);
        }

        if (y.first.first == 0) {
            yc = y.first.second;
        } else if (y.first.second == C) {
            yc = C + y.first.first;
        } else if (y.first.first == R) {
            yc = R + C + (C - y.first.second);
        } else if (y.first.second == 0) {
            yc = R + 2 * C + (R - y.first.first);
        }
