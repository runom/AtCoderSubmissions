...
vector<int64_t> get_inverse_list(int n, const int mod) {
...
int main() {
    ...
    auto inv = get_inverse_list(max_n, mod);

    vector<int64_t> fact(max_n + max_k + 1), fact_inv(max_n + max_k + 1);
    fact[0] = 1;
    fact_inv[0] = 1;

    for (int i : rep(1, max_n + max_k + 1)) {
        fact[i] = fact[i - 1] * i % mod;
    }

    for (int i : rep(1, max_n + 1)) {
        fact_inv[i] = fact_inv[i - 1] * inv[i] % mod;
    }

    auto comb = [&fact, &fact_inv](int n, int k) {
