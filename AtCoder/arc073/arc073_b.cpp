...
int main() {
    ...
    vector<vector<int64_t>> li(4);

    for (int i : rep(0, n)) {
        li[a[i] - a[0]].push_back(b[i]);
    }
    ...
    vector<vector<int64_t>> vsum(4);

    for (int i : rep(0, 4)) {
        vsum[i].push_back(0);
        for (auto x : li[i]) {
            vsum[i].push_back(x + vsum[i].back());
        }
    }

    int64_t ans = 0;
    for (int i : rep(0, vsum[0].size())) {
        for (int j : rep(0, vsum[1].size())) {
            for (int k : rep(0, vsum[2].size())) {
                for (int l : rep(0, vsum[3].size())) {
                    int64_t x = a[0] * i + (a[0] + 1) * j + (a[0] + 2) * k +
                                (a[0] + 3) * l;
                    int64_t y =
                        vsum[0][i] + vsum[1][j] + vsum[2][k] + vsum[3][l];
                    if (x <= w) {
                        ans = max(ans, y);
                    }
                }
