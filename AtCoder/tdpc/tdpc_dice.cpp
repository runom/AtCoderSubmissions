...
int main() {
    ...
    auto dp = make_vector<double, 4>(n + 1, a[2] + 3, a[3] + 2, a[5] + 2);

    dp[0][0][0][0] = 1.0;

    for (int i : rep(0, n)) {
        for (int j : rep(0, a[2] + 1)) {
            for (int k : rep(0, a[3] + 1)) {
                for (int l : rep(0, a[5] + 1)) {
                    dp[i + 1][j][k][l] += dp[i][j][k][l] / 6.0;
                    dp[i + 1][min(a[2], j + 1)][k][l] += dp[i][j][k][l] / 6.0;
                    dp[i + 1][j][min(a[3], k + 1)][l] += dp[i][j][k][l] / 6.0;
                    dp[i + 1][min(a[2], j + 2)][k][l] += dp[i][j][k][l] / 6.0;
                    dp[i + 1][j][k][min(a[5], l + 1)] += dp[i][j][k][l] / 6.0;
                    dp[i + 1][min(a[2], j + 1)][min(a[3], k + 1)][l] +=
                        dp[i][j][k][l] / 6.0;
                }
            }
        }
