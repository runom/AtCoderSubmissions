...
int main() {
    ...
    vector<int> sum1(s.size()), sum2(t.size());

    for (int i : rep(0, s.size())) {
        sum1[i] = (i != 0 ? sum1[i - 1] : 0) + (s[i] == 'A');
    }

    for (int i : rep(0, t.size())) {
        sum2[i] = (i != 0 ? sum2[i - 1] : 0) + (t[i] == 'A');
    }
    ...
    for (int i : rep(0, q)) {
        ...
        int sa = sum1[b] - (a != 0 ? sum1[a - 1] : 0);
        int sb = (b - a + 1) - sa;
        int ta = sum2[d] - (c != 0 ? sum2[c - 1] : 0);
        int tb = (d - c + 1) - ta;
        ...
        ta %= 3;
        tb %= 3;
        out((sb - sa + 3) % 3 == (tb - ta + 3) % 3 ? "YES" : "NO");
    }
}