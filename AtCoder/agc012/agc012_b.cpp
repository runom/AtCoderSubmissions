...
void bfs(int root, int d, int c) {
    queue<Query> que;
    que.push((Query){root, d, c});
    ...
    while (!que.empty()) {
        ...
        info[x.v] = max(info[x.v], x.d);

        if (x.d == 0) continue;

        for (int to : graph[x.v]) {
            que.push((Query){to, x.d - 1, x.c});
        }
    }
}
...
int main() {
    ...
    boost::reverse(queries);

    fill(info, -1);

    for (auto &query : queries) {
        bfs(query.v, query.d, query.c);
    }
