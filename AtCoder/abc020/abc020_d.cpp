...
int main() {
    ...
    auto divisor_list = divisor(k);

    vector<int> prime_factor_list;
    {
        auto tmp = prime_factor(k);
        for (auto &&x : tmp) {
            prime_factor_list.push_back(x.first);
        }
    }
    ...
    for (int d : divisor_list) {
        int kd = k / d;
        vector<int> factors;

        for (int factor : prime_factor_list) {
            if (kd % factor == 0) {
        ...
        for (int i : rep(0, 1 << factors.size())) {
            int64_t factor = 1;
            int cnt = 0;
            for (int j : rep(0, factors.size())) {
                if ((i >> j) & 1) {
                    cnt++;
                    factor *= factors[j];
                }
